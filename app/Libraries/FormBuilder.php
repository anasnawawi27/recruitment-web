...
class FormBuilder
{
    ...
    private $config       = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        //   'default_radio_class'            => 'form-check-input',
        'default_control_label_class'    => 'col-form-label col-md-2 col-sm-4',
        'default_no_label_class'         => 'col-md-offset-2',
        'default_form_control_class'     => 'col-md-5 col-sm-8',
        'default_form_class'             => '',
        'default_button_classes'         => 'btn btn-primary',
        'default_date_post_addon'        => '',
        // For instance '<span class="input-group-btn"><button class="btn default" type="button"><i class="glyphicon glyphicon-calendar"></i></button></span>'
    ...
    private $input_addons = array(
        'exists'    => false,
        /* does the specific input have an addon? */
        'pre'       => array(),
        /* container for pre addons */
        'pre_html'  => '',
        'post'      => array(),
        /* container for post addons */
        'post_html' => ''
    );
                ...
                switch ($this->func) {
                    ...
                    case 'form_radio':
                        ...
                        foreach ($all_elm_options['options'] as $elm_suboptions) {
                            ...
                            if (isset($this->elm_options['class']) && $this->elm_options['class'] == 'custom-control-input') {
                                $label_class = isset($this->elm_options['label_class']) ? $this->elm_options['label_class'] : 'custom-control ' . ($this->func == 'form_checkbox' ? 'custom-checkbox' : 'custom-radio');
                                array_key_exists('inline', $all_elm_options) && $label_class .= ' custom-control-inline';
                                array_key_exists('disabled', $this->elm_options) && $label_class .= ' disabled';

                                $this->print_string .= '<div class="' . $label_class . '">';
                                $this->print_string .= $this->_build_input(FALSE);
                                $this->print_string .= '<label class="custom-control-label" for="' . $this->elm_options['id'] . '">' . ($this->elm_options['label'] === '' ? '&nbsp;' : $this->elm_options['label']) . '</label></div>';
                            } else {
                                $label_class = isset($this->elm_options['label_class']) ? $this->elm_options['label_class'] : 'form-check pl-0 mt-1';
                        ...
                        $this->print_string .= $this->_build_help_block();
    ...
    private function _prep_options()
    {
        ...
        $this->elm_options_help = (isset($this->elm_options['help']) && !empty($this->elm_options['help'])) ? $this->elm_options['help'] : false;
    ...
    private function _build_input($include_pre_post = true)
    {
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_date':
                    ...
                    try {
                        if (empty($this->elm_options['value'])) {
                            ...
                            $dt = new time($this->elm_options['value']);
                            $this->elm_options['value'] = $dt->format($this->config['default_date_format']);
                        }
                    } catch (Exception $e) {
                ...
                case 'form_submit':
                    $label = (isset($this->elm_options['label']) ? $this->elm_options['label'] : '');
                    $class = str_replace($this->config['default_button_classes'], '', $this->elm_options['class']);
                    $class = str_replace($this->config['bootstrap_required_input_class'], '', $class);
                    if (strpos($class, $this->config['default_button_classes']) === FALSE) {
                        $class .= ' ' . $this->config['default_button_classes'];
                    }
                    $back_url = (isset($this->elm_options['back_url']) ? $this->elm_options['back_url'] : false);
                    $input_html_string = '';
                    if ($back_url) {
                        $back_label = (isset($this->elm_options['back_label']) ? $this->elm_options['back_label'] : 'Cancel');
                        // $input_html_string = '<a href="'.$back_url.'" class="text-danger">'.$back_label.'</a>';
                        $input_html_string = '<a href="' . $back_url . '" class="btn btn-light" style="color:black">' . $back_label . '</a>';
                    }
                    // $input_html_string .= '<button type="submit" class="'.$class.' ml-3">'.$label.'</button>';
                    $input_html_string .= '<button type="submit" class="' . $class . ' ml-1">' . $label . '</button>';
                    break;
                case 'form_option':
                case 'form_dropdown':
                case 'form_multiselect':
                    /* form_dropdown is different than an input */ //                    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
    ...
    private function _build_input_addons_pre()
    {
        ...
        if ($this->input_addons['exists']) {
            if (!empty($this->input_addons['pre_html'])) {
                ...
                $ret_string .= '<div class="input-group"><div class="input-group-prepend">';
                foreach ($this->input_addons['pre'] as $pre_addon) {
                    $ret_string .= '<span class="input-group-text">' . $pre_addon . '</span>';
                }
                $ret_string .= '</div>';
            }
        }
    ...
    private function _pre_input()
    {
        if (($this->func === 'form_date') && ($this->config['default_datepicker_class'] !== '')) {
            return '<div class="date ' . $this->config['default_datepicker_class'] . ' ' . $this->config['default_form_control_class'] . '" data-date="' . $this->elm_options['value'] . '" data-date-format="' . preg_replace(array(
                '/Y/',
                '/m/',
                '/d/'
            ), array(
                ...
                'mm',
                'dd'
            ), $this->config['default_date_format']) . '" data-date-viewmode="years">';
        }
        //        $this->elm_options['']
    ...
    private function _build_help_block()
    {
        if (!empty($this->elm_options_help)) {
            return '<small class="' . (isset($this->elm_options_help['class']) ? $this->elm_options_help['class'] : 'form-text text-muted') . '" ' . (isset($this->elm_options_help['id']) ? 'id="' . $this->elm_options_help['id'] . '"' : '') . '>' . $this->elm_options_help['text'] . '</small>';
        }
        return '';
    }
    ...
    private function _recursive_build_json($ary, $offset = 0)
    {
        ...
        foreach ($ary as $k => $v) {
            ...
            if ((is_array($v) || is_object($v)) && !is_string($v)) {
                ...
                $kv_str .= '<div class="' . $offset_class . '"><strong>' . ucwords(strtolower(str_replace(array(
                    '_',
                    '-'
                ), ' ', $k))) . '</strong>' . $innter_str . '</div>';
            } else {
                $kv_str .= '<div class="' . $offset_class . '"><strong>' . ucwords(strtolower(str_replace(array(
                    '_',
                    '-'
